...
flowchart LR
  ...
  subgraph Firebase["🔥 Firebase · asia-northeast3 (Blaze)"]
    ...
    FN["Cloud Functions<br/>listBaseBranches · createSpecPR<br/>closeSpecPR · githubWebhook<br/>notifyOnFeatureWrite"]
  end
...
| 역할 | 하는 일 | 가이드 |
|---|---|---|
| 🧑‍💻 **개발자** | `/issue`로 base 브랜치 → `/mino-spec` 작성 → 업로드·검증 → 컨펌 요청 → 반려 반영 → PR 생성 → 무효화 관리 | 📖 **[docs/role/DEVELOPER.md](docs/role/DEVELOPER.md)** |
