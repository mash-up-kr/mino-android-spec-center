...
### 4.9 PRD 트랙 — 상위 문서 (P8 · 구현 완료·배포 대기)

> 신설 (2026-08-15). 상세 설계는 **[design/prd-track.md](design/prd-track.md)** 가 최신이며, 이 절은 요구사항 수준의 요약이다.

spec-center의 문서 체계(1.2)는 지금까지 하위 문서(spec)만 다뤘다. P8은 그 **상위 문서인 PRD**(`docs/prd/business-context.md`, `/mino-prd` 산출물, **프로젝트당 1개**)를 대시보드로 끌어올린다.

| # | 요구사항 | 결정 |
|---|---|---|
| 1 | **PRD 업로드** | `/mino-prd` 산출물 파일 첨부 → 구조 검증 **P1–P7** → `prds/{id}` 저장 + 버전 스냅샷. 업로드 권한 **개발자 한정** |
| 2 | **전원 댓글** | `prds/{id}/comments` 서브컬렉션 — 개발자·디자이너 **누구나**, 섹션 앵커 인라인, 1단 답글, 본인 수정·소프트 삭제, `@` 자동완성 멘션. 상태 게이트 없음(상시 오픈) |
| 3 | **spec ↔ PRD 버전 호환** | spec 헤더 `**기준 PRD 버전**`(= `features.prdVersion`)이 조인 키. semver 등급으로 🔴비호환/🟡뒤처짐/⚪️무해 판정 — **표시 전용, 차단하지 않음** |
| 4 | **업로드/개정 Discord 알림** | `notifyOnPrdWrite` 트리거. 등급별 색 + **뒤처진 spec 목록 동봉**, `?prd={id}` 딥링크. 등록·MAJOR·MINOR 는 **Android·Design·iOS·Node 4개 역할 전부** 멘션(제품 전체 문서라 플랫폼을 가리지 않음), PATCH·본문 갱신은 무멘션. 댓글은 `notifyOnPrdComment` 가 **멘션 포함분만** |
| 5 | **버전 diff** | 버전별 본문 스냅샷 기반 임의 두 버전 비교 + 섹션 변경 요약 |

- **버전 소유권은 `/mino-prd` 스킬**이다 — spec과 같은 원칙(4.7)으로, 대시보드는 헤더 표의 `**버전**` 값을 **읽기만** 한다. MAJOR/MINOR/PATCH의 의미(하위 spec이 영향을 받는가)도 스킬의 판정 기준을 그대로 따른다.
- **PRD는 컨펌 게이트를 타지 않는다** — 승인 대상이 아니라 논의 대상이므로 상태 enum도, 파이프라인 상태머신(4.8) 변경도 없다.
- **PRD의 PR 자동 생성은 하지 않는다** — PRD 파일은 `/mino-prd`가 개발자 워킹트리에 직접 쓰므로 평소 PR 경로가 이미 있다. 대시보드가 두 번째 커밋 경로를 만들면 SoT가 갈린다. 대시보드의 역할은 *공유 아카이브 + 논의 + 호환 추적*으로 한정한다.
- **저장 구조가 spec과 다른 점**: 버전 스냅샷을 배열(`versionLog[]`)이 아니라 **서브컬렉션**에 둔다. PRD는 본문이 크고 개정이 잦아 Firestore 1MB 문서 한도가 현실적 위험이기 때문이다.
