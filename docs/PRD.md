...
| 2 | **전원 댓글** | `prds/{id}/comments` 서브컬렉션 — 개발자·디자이너 **누구나**, 섹션 앵커 인라인, 본인 수정·소프트 삭제. 상태 게이트 없음(상시 오픈) |
| 3 | **spec ↔ PRD 버전 호환** | spec 헤더 `**기준 PRD 버전**`(= `features.prdVersion`)이 조인 키. semver 등급으로 🔴비호환/🟡뒤처짐/⚪️무해 판정 — **표시 전용, 차단하지 않음** |
| 4 | **업로드/개정 Discord 알림** | `notifyOnPrdWrite` 트리거. 등급별 색·역할 멘션 + **뒤처진 spec 목록 동봉**, `?prd={id}` 딥링크 |
