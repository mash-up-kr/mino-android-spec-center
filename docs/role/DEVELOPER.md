...
- **spec.md 붙여넣기 또는 파일 드롭**(라이브 마크다운 프리뷰 제공)
- **대상 base 브랜치 선택** — `/issue`가 만든 `…/base` 목록이 GitHub에서 조회돼 셀렉트로 뜹니다. 선택하지 않으면 저장할 수 없습니다
- 출처 Figma 링크는 **본문 §1의 `**Figma**:` 줄에서 자동 수집**됩니다(수기 입력란 없음)
- 저장 시 **S1–S6 구조 검증**([validation.md](../design/validation.md)) 통과해야 생성됩니다. 실패하면 항목별 인라인 에러가 뜨고 저장이 막힙니다
- **경고(노란 박스)는 저장을 막지 않습니다** — 상태 `DRAFT`, 미해소 `[TBD]` 두 가지입니다. 그대로 올리고 검수에서 확정하세요
- 통과 시 `status = spec_draft`, 버전은 헤더 `**버전**` 값 그대로

> 검증 항목 요약: S1 대상 스펙 경로·제목 · S2 필수 H2 4개 · S3 헤더 메타(상태·버전·날짜·기준 PRD) · S4 FR/UX/SC ID · S5 유저 플로우 구조(진입/완료 조건·TS 표) · S6 템플릿 자리표시자 잔여 0건.
> 경고(비차단): 상태가 `CREATED`가 아님 · `[TBD]` 잔여.
...
- `docs/specs/{slug}/spec.md` 커밋 (plan·이미지 커밋 없음)
