...
> 짝 문서: [notifications.md](./notifications.md) — 여기서 만드는 상태 전이·새 논의 이벤트가 Discord 알림의 트리거가 된다.
...
- 알림(Discord) — 짝 문서 [notifications.md](./notifications.md) 소관.
...
| **@멘션** | 본문에서 `@handle` 추출 → `mentions[]` (알림 대상). MVP는 이름 텍스트, 실제 Discord 멘션은 [notifications.md](./notifications.md) Tier 2 |
...
- KPI/필터칩: **"논의 있음"**. ("내 멘션" 필터는 알림과 연동 — [notifications.md](./notifications.md))
...
- **알림은 별도 문서** — [notifications.md](./notifications.md)로 분리(2026-07-07).
