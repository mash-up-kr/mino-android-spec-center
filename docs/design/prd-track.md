...
→ **검증 전 HTML 주석 블록을 제거한 사본으로 판정한다.** 저장되는 `body` 는 원문 그대로다(뷰어 `mdToHtml` 은 이미 주석을 건너뛴다 — [app.js](../../js/app.js) `mdToHtml` 의 `inComment` 처리).
...
| spec 의 기준 PRD 버전 ↔ 현재 PRD 버전 | 등급 | 의미 |
|---|---|---|
| 동일 | ✅ 최신 | — |
| MAJOR 뒤짐 | 🔴 **비호환** | MVP 경계·용어 정의 변경 → `/mino-spec` **재실행 필요** |
| MINOR 뒤짐 | 🟡 뒤처짐 | 추가된 항목과 주제가 겹치는 spec 만 재실행 |
| PATCH 뒤짐 | ⚪️ 무해 | 표현·오타·링크 갱신 → 재실행 불필요 |
| spec 이 더 높음 | 🟠 이상 | PRD 업로드 누락 의심 — 최신 PRD 를 올려야 함 |
| `없음` · 파싱 실패 | ⚫️ 미연결 | PRD 이전에 작성된 spec |
...
| 이벤트 | 멘션 | 색 | 본문 |
|---|---|---|---|
| 신규 등록 | Android + Design | purple | 제품명 · 버전 |
| **MAJOR** 개정 | Android + Design | red | `1.0.0 → 2.0.0` + **뒤처진 spec 목록**(전부 재실행 대상) |
| **MINOR** 개정 | Android | amber | 뒤처진 spec 목록 (주제 겹침 확인 필요) |
...
- [x] `(FE)` 🔴 비호환 상태에서 `컨펌 요청` 시 확인 다이얼로그
